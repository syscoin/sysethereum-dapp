...
class Step1SX extends Component {
  ...
  async getTx() {
    ...
    try {
      ...
      this.setState({ buttonVal: false, buttonValMsg: (e.data ? e.data.message : undefined) ||  (e.data ? e.data.message : undefined) || e.message || e  });
      return;
    }
    ...
    if (valid === true) {
      ...
      try {
        let results = await this.sysToSysX(
