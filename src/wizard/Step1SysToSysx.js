...
class Step1SX extends Component {
  ...
  async sysToSysX (amount, xpub, sysChangeAddress) {
    ...
    const res = await this.syscoinjs.syscoinBurnToAssetAllocation(txOpts, assetMap, sysChangeAddress, feeRate, xpub)
    let err = null
    if (!res) {
  ...
  async getTx() {
    ...
    if(valid === true){
      ...
      try {
        let results = await this.sysToSysX(userInput.amount.toString(), xpub, sysChangeAddress);
        if(results.error){
        ...
        <div className="row">
          <form id="Form" className="form-horizontal">
            ...
            <label className="col-md-12">
                <h1 dangerouslySetInnerHTML={{__html: this.props.t("step1SXHead")}}></h1>
                <h3 dangerouslySetInnerHTML={{__html: this.props.t("step1SXDescription")}}></h3>
              </label>
              <div className="row">
