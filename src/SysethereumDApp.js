...
class SysethereumDApp extends Component {
  ...
  async componentDidMount() {
    ...
    // just a tip: you can disable the other buttons while there is not connected account to force the user to connect first
    // for lock/unlock pls call controller.isLocked() and create a conditional to show the user a message to unlock his wallet
    ...
    const callback = async (event) => {
      if (event.detail.SyscoinInstalled) {
        ...
        if (event.detail.ConnectionsController) {
          ...
          // initialize app with no account connected, clearing storage
    ...
    console.log("checking syscoin status", this);

    window.addEventListener("SyscoinStatus", callback);

    console.log("after syscoin update");
  }

  async connectWallet() {
    const storedAccount = localStorage.getItem("connectedAccount");

    const { controller, connectedAccount } = this.state;

    await controller.connectWallet();

    const account = await controller.getConnectedAccount();

    if (!connectedAccount || !storedAccount) {
      this.setState({
        ...this.state,
