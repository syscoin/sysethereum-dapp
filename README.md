...
2) You should be running [Syscoin Core](https://github.com/syscoin/syscoin) with `-blockindex` configuration set so that `getblockhashbytxid` can be queried for blocks based on txid needed for the Syscoin to Ethereum wizard. You should also configure the RPC parameters for the dapp to talk to Syscoin Core.
